...
da<-read.csv("resHHtoSH.csv")
#get just differentially abundant ones
#make a new category if they are diff abundant (=1) or not (=0)
new$diffab<-ifelse(new$feature1 %in% da$X, 1, 0)
new$diffab1<-ifelse(new$feature2 %in% da$X, 1, 0)
test<-new[which(new$diffab=='1'|new$diffab1=='1'),]
...
plot(clp,test_graph_simple,vertex.color=colrs[V(test_graph)$community],edge.label=test$r, main="Differentially abundant between naive and exposed, |r|>=0.5")
