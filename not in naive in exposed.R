#this works but its useless
library(igraph)
library(dplyr)
new<-read.csv("SH5.csv")
h<-read.csv("HH5.csv")
#get just the ones that are in SH
new$present1<-ifelse(new$feature1 %in% h$feature1, 0, 1)
new$present2<-ifelse(new$feature2 %in% h$feature2, 0, 1)
test<-new[which(new$present1=='1'& new$present2=='1'),]
...
test<-subset(test, r >= 0.3 | r <= (-0.3), select=3:7)
...
plot(clp,test_graph_simple,vertex.color=colrs[V(test_graph)$community],edge.label=test$r, main="in exposed, not in naive, |r|>=0.3")
